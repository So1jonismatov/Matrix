...
START_TEST(test_calc_complements_invalid) {
  matrix_t m, res;
  s21_create_matrix(2, 3, &m);
  ck_assert_int_eq(s21_calc_complements(&m, &res), S21_RESULT_ERROR);
  s21_remove_matrix(&m);
}
END_TEST
...
START_TEST(test_calc_complements_1x1) {
  ...
  ck_assert_int_eq(s21_calc_complements(&m, &res),
                   S21_RESULT_ERROR);  // Should fail for 1x1
  s21_remove_matrix(&m);
}
END_TEST
...
START_TEST(test_determinant_invalid) {
  ...
  s21_create_matrix(2, 3, &m);
  double det;
  ck_assert_int_eq(s21_determinant(&m, &det), S21_RESULT_ERROR);
  s21_remove_matrix(&m);
}
END_TEST
